...
void run_all_dist(int id = -1) {
  ...
  for (auto v : num_keys) {
    ...
    // 使用相同的键值对测试 semi-sort
    printf("Running Semi-Sort on Uniform Distribution, keys=%zu\n", v);
    run_all(seq, id);

    // 使用相同的键值对测试 group_by_key_ordered
    printf("Running GroupByKeyOrdered on Uniform Distribution, keys=%zu\n", v);
    auto key_vals = parlay::delayed_map(seq, [](auto x) { return std::make_tuple(x.first, x.second); });
    internal::timer t;
    auto result = parlay::group_by_key_ordered(key_vals);
    t.stop();
    printf("GroupByKeyOrdered (Uniform Distribution, keys=%zu): %f seconds\n", v, t.total_time());
  }
  ...
  for (auto v : lambda) {
    ...
    // 使用相同的键值对测试 semi-sort
    printf("Running Semi-Sort on Exponential Distribution, lambda=%f\n", v);
    run_all(seq, id);

    // 使用相同的键值对测试 group_by_key_ordered
    printf("Running GroupByKeyOrdered on Exponential Distribution, lambda=%f\n", v);
    auto key_vals = parlay::delayed_map(seq, [](auto x) { return std::make_tuple(x.first, x.second); });
    internal::timer t;
    auto result = parlay::group_by_key_ordered(key_vals);
    t.stop();
    printf("GroupByKeyOrdered (Exponential Distribution, lambda=%f): %f seconds\n", v, t.total_time());
  }
  ...
  for (auto v : s) {
    ...
    // 使用相同的键值对测试 semi-sort
    printf("Running Semi-Sort on Zipfian Distribution, s=%f\n", v);
    run_all(seq, id);

    // 使用相同的键值对测试 group_by_key_ordered
    printf("Running GroupByKeyOrdered on Zipfian Distribution, s=%f\n", v);
    auto key_vals = parlay::delayed_map(seq, [](auto x) { return std::make_tuple(x.first, x.second); });
    internal::timer t;
    auto result = parlay::group_by_key_ordered(key_vals);
    t.stop();
    printf("GroupByKeyOrdered (Zipfian Distribution, s=%f): %f seconds\n", v, t.total_time());
  }
  ...
  for (auto v : rate) {
    ...
    // 使用相同的键值对测试 semi-sort
    printf("Running Semi-Sort on Bits Exp Distribution, rate=%zu\n", v);
    run_all(seq, id);

    // 使用相同的键值对测试 group_by_key_ordered
    printf("Running GroupByKeyOrdered on Bits Exp Distribution, rate=%zu\n", v);
    auto key_vals = parlay::delayed_map(seq, [](auto x) { return std::make_tuple(x.first, x.second); });
    internal::timer t;
    auto result = parlay::group_by_key_ordered(key_vals);
    t.stop();
    printf("GroupByKeyOrdered (Bits Exp Distribution, rate=%zu): %f seconds\n", v, t.total_time());
  }
}
